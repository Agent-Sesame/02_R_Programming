rankhospital <- function(state, outcome, num = "best") {
        ...
        if (!state %in% unique(dat[, 3])) { stop("invalid state") }
        if (!outcome %in% definedOutcomes) { stop("invalid outcome") }
        ...
        if (outcome == "heart attack") {
                ...
                dat <- dat[order(as.numeric(dat$`Heart Attack`), dat$Hospital),]
                rownames(dat) <- NULL
                if (num == "best") { num <- 1 }
                if (num == "worst") { num <- dim(dat)[1] }
                print(dat[num, 2])
        }
        ...
        if (outcome == "heart failure") {
                ...
                dat <- dat[order(as.numeric(dat$`Heart Failure`), dat$Hospital),]
                rownames(dat) <- NULL
                if (num == "best") { num <- 1 }
                if (num == "worst") { num <- dim(dat)[1] }
                print(dat[num, 2])
